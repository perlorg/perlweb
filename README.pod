...
You'll need Apache 2.x (no mod_perl), Apache 1.3.x with mod_perl
1.2.x, Perl 5.8 and the CPAN modules from the following section.
Combust can be installed and run as any user, but the Apache 2.x
installation must be started as root.
...
install Date::Parse DBI Exception::Class Bundle::LWP Text::Template
install Config::Simple Mail::Internet Time::HiRes 
install URI::Find DBD::mysql Template Apache::Reload
...
feature.  If for development you just want to run Combust on a high
