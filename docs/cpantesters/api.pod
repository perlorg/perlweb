...
In addition to the api_key each user also has an C<api_secret>.  On most requests a C<ts> and a C<sig> parameter is required, too.   The C<ts> is a timestamp and C<sig> is 
