...
=item All object creation is checked with C<isa_ok()>.
...
=item All temporary files are cleaned up.
...
=item All tests should have a name.

The last argument in all the Test::More functions is for the tests
name/short-description. If you write a test you should know what it is
doing, so give it a name/short-description.
...
=item Object Equaivalence testing
...
instances, and C<is> and C<isnt> compare strings.

=item Use the right function for the job
...
should be removed and replaces with:
...
=item Use C<is_deeply> to test things other than scalars
...
=item Test B<all> your assumptions.
