...
easierE<8212>presuming we don't have to un-invent anything between now
and then...)
...
based on the types of one or more of its arguments.
...
definitions, which are rather heavyweightE<8212>not to mention opaque.
...
Which is fineE<8212>that's why it's optional.
...
(strings) to cats.  Of course, that's an oversimplificationE<8212>for both
...
before the long partsE<8212>linguists call this the "end-weight" problem.)
...
The Rest" list of values that many functionsE<8212>like C<print>, C<map>,
and C<chomp>E<8212>have at the end of their call.  Whereas positional
...
between positional and variadic dataE<8212>you just have to declare them
...
underlying metaphor really is very much like zoning regulationsE<8212>you
...
distinguished in the callE<8212>you just use commas.  We'll discuss call
...
parameters).  Since they areE<8212>by definitionE<8212>in the variadic region,
...
to occur before the list arguments for efficiencyE<8212>otherwise the
...
You don't really care exactly where a factory isE<8212>as long as it's not
in your back yardE<8212>you care what it produces.  The typical factory is
...
an implicitly declared slurpy hashE<8212>known as C<%_> by analogy
to C<@_>E<8212>to handle surplus named arguments).
...
That's still a lot of extra unnecessary cruftE<8212>but we can do better.
...
parametersE<8212>the general rule of thumb for an initializer is that you
...
called automatically most of the timeE<8212>but not necessarily all the
...
all the subroutine linkageE<8212>but not the lexical linkage.  It can only
...
pure syntactic sugarE<8212>except that we embrace syntactic sugar in Perl
...
I define "small number" as approximately I<e> E<177> E<960>.  But as
...
Perl 6 doesn't go quite that farE<8212>you must declare your multimethods
...
are usedE<8212>there are no forward declarations of macros, and while a
...
list operator wouldE<8212>that is, as a comma-separated list with the same
...
all-caps any more.  We know we have a C<Scalar> typeE<8212>hopefully we also
...
You can even curry built-in operators:
    ...
    my &prefix:E<frac12> ::= &infix:/ .assuming(y => 2);
...
to associativityE<8212>they always go from the outside in.)
...
The purpose of an lvalue subroutine is to return a "proxy"E<8212>that is,
...
underneathE<8212>especially if that calculation entails side effects.
(Or is expensiveE<8212>meaning that it has the side-effect of chewing up
computer resources...)
...
C<temp> and C<let> comes fromE<8212>they're fire-and-forget operators.
...
espouse lvalue subsE<8212>all of which were rejected. C<:-)>
...
been stolen.  Please don't take any of this personallyE<8212>I ignore
...
to call the method twiceE<8212>once to get the old value, and once to set
the new value.
...
*are* list operatorsE<8212>but not all of them are.  If you say:
...
away the specsE<8212>though perhaps in this case the range specs can
...
like to see how sausage is made.  C<:-)>  E<8212>Larry]
