...
[Update: We now also have C<«...»> for capturing assertions.]
...
[Update: Use C<«$rule»> to turn any non-capturing rule into a capturing
...
[Update: Rules are not captured by default.  You can use C<:=> or
use C<«...»> instead of angles to force capture.]
...
[Update: The C<$0> variable is split up into a C<$/> variable that represents
the match state object, and a C<$0> variable that represents the capture
of the entire pattern.  In other words, C<$0> is exactly like a C<$1>
capture, only with assumed parens around the entire pattern.]
...
[Update: This restriction is no longer in effect.  Any variable may
be treated hypothetically, not just variables stored in C<$/>.
The C<$/> variables are now syntactically distinguished from ordinary
variables.  Hypotheticality is more or less orthogonal to that.]
...
you can't store two different types under the same name.]
...
[Update: Any subrule may be captured with French quotes.]
...
[Update: C<$0> is still the entire match, but the match object is now
C<$/>, and a list of all beginnings is returned by the hyperoperator
C<$/».beg>.  But note that string positions are not necessarily integers
in Perl 6.  They are tagged with the units of the string, so that you
can't inadvertently mix byte, codepoint, or grapheme offsets.]
