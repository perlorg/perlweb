...
[Update: the range hyphens are now replaced by C<..> to be consistent with
ranges elsewhere in Perl 6.]
...
[Update: The C<@{...}> construct is now C<@(...)>.  A C<.list> would
probably be more readable though.  And smartmatch is C<~~> now.]
...
[Update: Now you can just set the desired scalar result object with a
C<make> call in the closure.]
...
[Update: The generalized repetition is now C<**{$n..$m}>, and the braces may be
omitted for literal numbers, so that C<< <out_marker><3> >>
would now be written C<< <out_marker> ** 3 >>.]
