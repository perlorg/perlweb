...
Parrot (the virtual machine on which Perl 6 will run) can compile your
program to bytecode and -- on commonly used platforms -- to native
executable files. So Perl 6 will be exactly like every other compiled
...
Maybe. The parrot bytecode requires the parrot to run it, but if you are in
desperate need for a stand alone executable, you can statically link parrot
and your program to a single executable.
...
That said, there is much progress being made in the last year, 
including the Pugs project which allows you to start testing 
experimenting with some perl 6 features .. now! 
