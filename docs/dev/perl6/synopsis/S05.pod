...
If followed by an C<st>, C<nd>, C<rd>, or C<th>, it means find the
I<N>th occurrence.  Use C<:nth(3)> for the general form.  So
...
C<^^> and C<$$> match line beginnings and endings. (The C</m> modifier
is gone.)

=item *

C<.> matches an "anything", while C<\N> matches an "anything except
newline". (The C</s> modifier is gone.)
...
not allowed before the opening curly.  The curlies are taken to be
a closure returning a number or a range.
...
as a literal, or fails if no key matches.  (If there is a null key
it will match anywhere, provided no longer key matches.)  If the
corresponding value of the hash element is a closure, it is executed.
...
That closure is unlikely to run if the string doesn't end with "bar".
...
rare cases that needed them you can use C<< <{ lc $rule }> >> etc.
...
Otherwise it's a rule constructor.
...
As a general rule, if you capture anything repeating (even potentially)
to a scalar location, you end up with a list reference as the scalar value.
This always puts a list into the scalar:
...
By default, subrules (e.g. C<< <rule> >>) do not capture
their results.  To capture, use French quotes instead of angles (no
pun intended):
...
is visible.  (But it is illegal to use the same name for two different
capture datatypes.)
