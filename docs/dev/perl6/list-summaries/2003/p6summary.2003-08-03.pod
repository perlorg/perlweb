=head1 Perl 6 Summary for the week ending 20030703
...
L<http://groups.google.com/groups?threadm=3F2473A4.EED8FE8D%40hotpop.com>

L<http://groups.google.com/groups?threadm=Pine.LNX.4.44.0307281006310.18437-100000%40hydrogen.sabren.com>

=head2 Semantics of clone for PIO-objects

JE<uuml>rgen BE<ouml>mmels' work on the Parrot IO system continues to
...
JE<uuml>rgen had some questions about how to handle various things
like what to do with buffering or position pointers.
...
L<http://groups.google.com/groups?threadm=m24r167p89.fsf%40helium.physik.uni-kl.de>

=head2 Making 'make' less verbose

Leo TE<ouml>tsch checked in a patch to make make's output rather less
...
L<http://groups.google.com/groups?threadm=3F25264A.4070506%40nextra.at>

=head2 Don't trace system areas in sweep ops
...
L<http://groups.google.com/groups?threadm=3F252519.9060406%40nextra.at>
...
L<http://groups.google.com/groups?threadm=BAY1-DAV50Vo2Zz8dmY0000ac49%40hotmail.com>

=head2 PHP/Parrot

The language implementation insanity continues! 

Stephen Thorne announced that he's working on implementing a PHP
...
little about bootstrapping as well.

Luke Palmer and Simon Glover were forthcoming with lots of useful
answers and pointers.

L<http://groups.google.com/groups?threadm=200307291609.46294.stephen%40mu.com.au>

=head2 Why Parrot uses Continuation Passing Style

In a delayed response to a question from Klaas-Jan Stol, Dan has
...
(and the reason that my copy of I<Perl 6 Essentials> has a signed
correction from Dan).

L<http://groups.google.com/groups?threadm=a0521060ebb4c371912c0%40%5B63.120.19.221%5D>

=head2 IMCC supports the Parrot Calling Conventions

Leo announced that IMCC's brand of assembler, PIR (I can't remember
...
somewhat lacking on the test front, but it's a start. Yay Leo!

L<http://groups.google.com/groups?threadm=3F26B9E6.9020609%40nextra.at>

=head2 Another task for the interested: Coverage

Dan threw out another 'task for the interested' this week. At present
...
generated as part of the C<make test> run) would be very welcome. 

It turns out that Leo already has an "unportable, ugly, slightly
...
L<http://groups.google.com/groups?threadm=a05210610bb4c910e2831%40%5B63.120.19.221%5D>

L<http://www.hitchhiker.org/parrot_coverage> -- Josh's reports

=head2 Pirate (py...rrot)

Will the terrible jokes never stop?

Michal Wallace reported to the list on his attempts to retool
amk's F<parrot-gen.py> to generate code for IMCC. It sounds like he's
making good progress, but his choice of codename -- Pirate, from
py...rrot -- had at least one summarizer groaning. 

Later Michal asked the list about the best way of generating
...
Leo came up with a suggestion about code layout for Michal and spoke
for everyone (I think) when he added: 
...
euclid, but I'm sure we'll get that fixed soon enough.

L<http://groups.google.com/groups?threadm=Pine.LNX.4.44.0307300400420.25808-100000%40hydrogen.sabren.com>

L<http://groups.google.com/groups?threadm=Pine.LNX.4.44.0307310652190.6807-100000%40hydrogen.sabren.com>

=head2 JVM->PIR translator

Just as we were all giving Michal some good Wow, Joseph Ryan
announced that he had a partially complete JVM->PIR translator
done, though it still had a few issues.

L<http://groups.google.com/groups?threadm=3F2A11BF.9000906%40osu.edu>

L<http://jryan.perlmonk.org/images/jirate.tar.gz

=head2 Dynamic PMC Classes

Leo announced that he's started working on dynamic PMC classes. The
...
tricks. He already has something working, and asked for comments.
...
what Leo had implemented, but they don't call it B<soft>ware for
nothing. 
...
L<http://groups.google.com/groups?threadm=3F27A15C.3060301%40nextra.at>

=head2 Question about C<interpreter == NULL>

JE<uuml>rgen BE<ouml>mmels wondered which functions allowed the
...
will make it into documentation at some point.

L<http://groups.google.com/groups?threadm=m2ispin3v2.fsf%40helium.physik.uni-kl.de>

=head2 Adding C<yield> semantics to IMCC

Kenneth A Graves has been experimenting with the C<.pcc_*> directives
for implementing function calls, and wants to add coroutine support
by implementing C<.pcc_begin_yield> and C<.pcc_end_yield> which would
be analogous to the current C<.pcc_*_return> directives. He supplied
a patch implementing what he was after. Leo liked the patch and
applied it.

L<http://groups.google.com/groups?threadm=rt-23186-62028.2.9714262157627%40rt.perl.org>

=head2 IMCC objects speed, C<.include>, file-scoped vars et cetera

Now that Parrot nearly has objects, Jerome Quelin has started work on
...
Melvin Smith opined that the time had come to start putting together
...
Leo TE<ouml>tsch meanwhile answered most of Jerome's questions.

L<http://groups.google.com/groups?threadm=200307312302.13304.jquelin%40mongueurs.net>

=head2 IMCC's C<call> vs first class functions

If you were still not sure of the virtues of Continuation Passing
...
L<http://groups.google.com/groups?threadm=Pine.LNX.4.44.0308011305360.23256-100000%40hydrogen.sabren.com>

=head2 David Adler Scares Himself
...
L<http://groups.google.com/groups?threadm=20030802054905.GA22806%40panix.com>

=head2 Upcoming backwards incompatible changes to IMCC

Leo TE<ouml>tsch announced some changes to IMCC which will mean it is no
...
adding a new C<.globalconst> directive for declaring file scoped
constants. 

L<http://groups.google.com/groups?threadm=3F2B7BA0.9020500%40nextra.at>
...
L<http://groups.google.com/groups?threadm=Pine.LNX.4.44.0308021218250.18912-100000%40booger.sixgeeks.org>

=head1 Meanwhile, in perl6-language

Things are starting to warm up a little in perl6-language
following the publication of Exegesis 6 (take a look, you'll find it
at L<http://www.perl.com/pub/a/2003/07/29/exegesis6.html>, there's
...
=head2 Perl 6's C<for()> signature

John Siracusa referred back to an earlier summary where I had
wondered if either of two C<for> implementations had got the
...
with an accurate signature for C<for> (or at least, not one that
...
slurpy lists have to be the last parameter in the signature.

John countered by quoting from Exegesis 6 "An important goal of Perl
...
there's always the possibility of implementing something C<for>-like
using a macro, but that doesn't feel too comfortable.

Rod Adams proposed "non-greedy slurpy arrays" which would be
analogous to non-greedy regex matches and proposed C<*?@> as the
sigil combination for such a parameter. (Perl? Line noise? Never!)

Damian Conway tweaked Simon Cozens' "Soylen^WPerl 6 is Ruby!"
detector when he mentioned that Larry was considering adding a
special case for allowing a single C<&block> parameter after a slurpy
parameter, but that both Larry and Damian weren't entirely happy with the
idea. 

Larry offered words of wisdom. As usual.

L<http://groups.google.com/groups?threadm=F2B0434A-C36C-11D7-961B-0050E4A0A466%40mindspring.com>

L<http://www.perl.com/pub/a/2003/04/p6pdigest/20030427.html?page=2>
-- the earlier summary

L<http://groups.google.com/groups?threadm=20030802010217.GA17906%40wall.org> -- Larry dispenses wisdom

=head2 Exegesis 6: Assume nothing
...
curried function would reflect that. 
...
using C<.assuming> at all and just writing a simple wrapper function
by hand may be the way forward.

L<http://groups.google.com/groups?threadm=Pine.BSF.4.51.0307301958180.78431%40bowser.eecs.harvard.edu>

=head2 Mandating name-only parameters

Mark J. Reed wondered if the new parameter declaration syntax meant
...
than the single character prefixes. 

L<http://groups.google.com/groups?threadm=20030801154454.GA4016%40charm.turner.com>

=head2 Small Junctions

Exegesis 6 describes a junction as "a single scalar value that can
...
unsure as to the semantics for a 0 member junction. 

Luke Palmer gave a good answer, and pointed at Damian's message about
"Perl 6 and Set Theory" for more detail.

L<http://groups.google.com/groups?threadm=20030801165457.98906.qmail%40onion.perl.org>

L<http://groups.google.com/groups?hl=en&lr=&ie=UTF-8&oe=UTF-8&safe=off&selm=3DF2FE76.6050602%40conway.org&rnum=2>

=head2 Macros and C<is parsed>
...
answer from Larry appears to be that macros get a default C<parsed>
trait, which can be overridden by C<is parsed> when the macro is
declared, so macro arguments are parsed by the macro's C<parsed>
trait.

L<http://groups.google.com/groups?threadm=Pine.GSO.4.53.0308020838010.14060%40school.cs.indiana.edu>

=head2 Macro arguments themselves

Luke Palmer wondered what macros do about being passed variables,
...
recursive macro you would probably have to write a helper function.

L<http://groups.google.com/groups?threadm=ygcwudvc1hy.fsf%40babylonia.flatirons.org>

=head2 Another macro question
...
Which would throw an error under use strict. It seems to me that the
way to get expanded code that looks like:

    my $x = 7;
    print $x;

would be to declare C<foo> as:

    macro foo() { return 'my $x = 7' }

L<http://groups.google.com/groups?threadm=Pine.GSO.4.53.0308022223550.20685%40school.cs.indiana.edu>

=head2 C<grep EXPR, LIST>

John Williams wondered if the Perl 5ish C<grep EXPR, LIST> would
...
this summary are too narrow to contain my solution. 

L<http://groups.google.com/groups?threadm=Pine.LNX.4.33.0308022138390.18148-100000%40sharkey.morinda.com>
...
Ooh look, another plug for L<http://pc1.bofhadsl.ftech.co.uk:8080/>.
...
in this bit to L<p6summarizer@bofh.org.uk>
